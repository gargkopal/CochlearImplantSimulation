...
% 3.1. Create a program to read these files (or some of these files) into Matlab and find their sampling rate.
% (Hint: You can read the Matlab help pages about the command audioread1
% .) (see 3.6 below before
% starting this task).
...
% 3.2. Use size to check whether the input sound is stereo (2 channels/columns) or mono (1
% channel/column). Use if: If stereo, add the two columns to make it single channel (or a 1-column array).
...
% 3.5. Plot the sound waveform of one of your sound files as a function of the sample number.
% plot(yMono)


% 3.6. If the sampling rate of the input signal is not 16 kHz, downsample it to 16 kHz (resample). (If the
% original sampling rate is less thaan 16 kHz, it is much better to redo 3.1)



% % 3.7. Generate a signal using the cosine function (cos) that oscillates at 1 kHz with the same time duration
% % and array length (Task 3.2) as the input signal (you know the sampling rate, right?). Play the sound generated by signal and plot two cycles of its waveform as a function of time.
